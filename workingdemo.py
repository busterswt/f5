...
def delete_pool(bigip, partition, name):
    return bigip.delete('%s/ltm/pool/~%s~%s' % (BIGIP_URL_BASE, partition, name))

def delete_virtual(bigip, partition, name):
    return bigip.delete('%s/ltm/virtual/~%s~%s' % (BIGIP_URL_BASE, partition, name))
...
# delete virtual
try:
    resp = delete_virtual(bigip, PARTITION, VS_NAME)
    resp.raise_for_status()
...
# delete pool
try:
    resp = delete_pool(bigip, PARTITION, POOL_NAME)
    resp.raise_for_status()
